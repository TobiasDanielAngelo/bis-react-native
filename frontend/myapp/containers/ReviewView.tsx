...
import { CustomerViewItem } from "../components/SalesComponents/CustomerViewItem";
    ...
    props.visible && (
      <ReviewContext.Provider value={{ items: dataList }}>
        ...
        >
          ...
          <Icon name="edit" size={35} color="white" style={{ margin: 10 }} />
        </View>
            ...
            {customers
              ...
              .map((s) => (
                <TouchableOpacity
                  key={`$custView-${s.id}`}
                  onPress={() => {
                    s.id === customer ? setCustomer(-1) : setCustomer(s.id);
                  }}
                >
                  <CustomerViewItem
                    customer={s}
